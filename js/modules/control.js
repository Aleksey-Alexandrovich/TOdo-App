...
const {renderTodo, renderNumbers} = render;
...
const getUserName = () => document.querySelector('#username').innerHTML;
...
const createTask = (text, id, status) => {
  const tempId = id || Math.random().toString().substring(2, 10);
  const tempStatus = status || false;
